...
airport -s > out.log            # saving airport output to a file
awk -v white="$White" 'NR==1 { print white $0}' out.log | column -t #print only the firt line and align them

#Assign colors variables to new variables inside awk and print each column with different color
awk  -v red="$red"  -v yellow="$Yellow" -v cyan="$Cyan" -v purple="$Purple" -F" "  'NR>=2 {print cyan NR-1 " " red $1 " " yellow $2 " " cyan $3 " " purple $4 " " red $5 " " $6 " " "\033[0m" $7 " " $8 }' out.log | column -t
